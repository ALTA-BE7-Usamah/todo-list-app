...
func (th *TaskHandler) GetTaskByIdIDHandler() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, helper.ResponseSuccess("success get task", task))
	}
}
...
func (th *TaskHandler) UpdateTaskHandler() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, helper.ResponseSuccess("success update task", responseTask))
	}
}
...
func (th *TaskHandler) CompletedTaskHandler() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, helper.ResponseSuccess("success completed task", responseTask))
	}
}
...
func (th *TaskHandler) ReopenTaskHandler() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		task, rows, err := th.taskUseCase.ReopenTask(uint(id), uint(idToken))
		if err != nil {
			return c.JSON(http.StatusInternalServerError, helper.ResponseFailed("id not recognise"))
		}
		if rows == 0 {
		...
		return c.JSON(http.StatusOK, helper.ResponseSuccess("success reopen task", responseTask))
	}
}
